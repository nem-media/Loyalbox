-- 0046 — POINT TÆLLER MED, NÅR ET MEDLEM KALDES INAKTIVT
--
-- FEJLEN. `ryd_op_efter_frister()` afgør, om et medlem er inaktivt, ved at
-- tage den seneste af fem datoer: oprettelsen, stempler, medlemskaber,
-- belønninger og rabatter. Alle fem hører til STEMPELKORTET. Pointprogrammet
-- kom til i 0044/0045 med sine egne tabeller, og ingen af dem blev talt med.
--
-- KONSEKVENSEN er ikke teoretisk: en butik, der kun kører pointprogram, har
-- medlemmer helt uden rækker i de fem kilder. For dem er "sidste aktivitet"
-- lig med `created_at`, og 24 måneder efter tilmeldingen bliver de slettet —
-- mens kunden stadig handler og stadig samler point. Saldoen og hele
-- ledgeren følger med, fordi kontoen hænger på `loyalty_members` med
-- `on delete cascade`.
--
-- DET ER SAMME KLASSE FEJL SOM DE ANDRE, PUNKTPROGRAMMET AFDÆKKEDE: en
-- invariant, der holdt, så længe loyalitet kun betød ét stempelkort. Den
-- kunne ikke ses fra koden — den ligger i en SQL-funktion, og den rammer
-- først 24 måneder efter, at nogen tog pointprogrammet i brug.
--
-- KROPPEN ER 0012's EGEN, med de to kilder lagt til. Fristerne er urørte, og
-- variabelnavnene er de samme, så `opbevaring.test.ts` stadig kan parre dem
-- med `FRISTER` i src/lib/opbevaring.ts.
--
-- IDEMPOTENT: `create or replace`. Kan køres igen uden virkning.
...
begin
  ...
  -- 3) Medlemmer uden aktivitet i 24 måneder — stempelkort OG point.
  --
  ...
  with sidste_aktivitet as (
    select m.id,
           greatest(
             ...
             -- POINT TÆLLER OGSÅ SOM AKTIVITET.
             -- Uden de to her så en kunde, der KUN samler point, ud som om
             -- der aldrig var sket noget på kortet: alle fem øvrige kilder er
             -- stempelkortets. En butik, der kun kører pointprogram, ville
             -- derfor få hver eneste kunde slettet 24 måneder efter
             -- tilmeldingen — midt i et aktivt kundeforhold, og med saldoen
             -- med sig, fordi kontoen hænger på medlemmet med cascade.
             coalesce((select max(pt.created_at)
                         from public.loyalty_point_transactions pt
                        where pt.member_id = m.id), '-infinity'::timestamptz),
             coalesce((select max(greatest(pa.created_at, pa.updated_at))
                         from public.loyalty_point_accounts pa
                        where pa.member_id = m.id), '-infinity'::timestamptz)
           ) as sidst
...
$$;
-- Rettighederne igen. `create or replace` bevarer dem, men de skrives ud, så
-- en læser kan se, hvad der gælder, uden at slå to migrationer op — og så en
-- gendannelse fra denne fil alene ikke efterlader funktionen åben.
...
comment on function public.ryd_op_efter_frister(boolean) is
  'Sletter og anonymiserer efter opbevaringsfristerne. p_toerloeb=true tæller kun. Kaldes fra /api/cron/oprydning. Aktivitet tæller både stempelkort og point.';
