...
export function BestilUdenKontoForm({ product }: { product: Product }) {
  ...
  const [hex, setHex] = useState("#51624a");
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="box-shape space-y-4 border border-border bg-card p-5">
        ...
        <Field
          label="CVR-nummer"
          hint={fejl.cvr ?? "Otte cifre. Vi sælger kun til virksomheder."}
        >
          <Input name="cvr" inputMode="numeric" placeholder="12345678" required />
        </Field>

        <Field
          label="E-mail"
          hint={fejl.email ?? "Hertil sender vi kvittering og besked, når skiltet er afsendt."}
        >
      ...
      <div className="box-shape border border-border bg-card p-5">
        ...
        {fejl.antal ? <p className="mt-2 text-sm text-danger">{fejl.antal}</p> : null}
      </div>
      ...
      <div className="box-shape border border-border bg-card p-5">
        ...
        {egenFront ? (
          <div className="mt-4 flex items-center gap-3">
            <input
              type="color"
              value={normaliserHex(hex) ?? "#51624a"}
